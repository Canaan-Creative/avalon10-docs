...
| 返回数据举例            | STATUS=I,When=8337,Code=118,Msg=ASC 0 set info:<br/>pool success set to stratum+tcp://btc.ss.poolin.com:443 worker is cctrix.001 workerpassword is 123 Please reboot miner to make config work. ,Description=cgminer 4.11.1 |
...
| 命令说明                | 设置矿机IP地址。                                             |
| ----------------------- | ------------------------------------------------------------ |
| 命令格式                | ascset\|0,ip,\<mode\>,\<ip\>,\<mask\>,\<gw\>                 |
| 参数说明                | \<mode\>,s：静态IP模式。d：动态获取IP（DHCP）。<br>\<ip\>,矿机IP地址，仅在配置为静态IP模式才有效。<br/>\<mask\>,子网掩码，仅在配置为静态IP模式才有效。<br/>\<gw\>，网关，仅在配置为静态IP模式才有效。 |
| 举例（使用linux socat） | 将IP为192.168.189.135的矿机配置为静态IP：192.168.193.223<br/>echo -n   "ascset\|0,ip,s,192.168.193.223,255.255.255.0,192.168.193.1" \| socat -t 30 stdio   tcp:192.168.189.135:4028,shut-none && echo<br/>将IP为192.168.189.135的矿机配置为动态IP（DHCP）<br/>echo -n   "ascset\|0,ip,d" \|socat -t 30 stdio   tcp:192.168.189.135:4028,shut-none && echo |
| 返回数据举例            | STATUS=I,When=20159,Code=118,Msg=ASC 0 set info: ...         |
...
| 命令说明                | 设置矿机DNS地址。                                            |
| :---------------------- | ------------------------------------------------------------ |
| 命令格式                | ascset\|0,dns,\<DNS1>,\<DNS2\>                               |
| 参数说明                | \<DNS1>,首选DNS地址。<br>\<DNS2\>，备用DNS地址。             |
| 举例（使用linux socat） | 将IP为192.168.189.135的矿机的DNS地址配置为8.8.8.8，备用DNS为114.114.114.114<br/>echo -n   "ascset\|0,dns,8.8.8.8,114.114.114.114"\|socat -t 30 stdio   tcp:192.168.189.135:4028,shut-none && echo |
